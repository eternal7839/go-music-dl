...
- 🚰 **流式转发，高效省内存**: 保留了 **汽水音乐** 源的旧逻辑（因为涉及音频解密，必须下载完整数据），但对其他所有通用源（网易云、QQ音乐、B站等）实现了**流式转发**和 **Range 支持**。这意味着：
  - **秒开播放**: 用户无需等待完整下载即可开始播放
  - **极低内存**: 无论多少人同时下载，服务器内存占用极小
  - **支持拖动**: 浏览器进度条拖动时，服务器只转发请求的字节范围
